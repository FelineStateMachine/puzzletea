...
Takuzu+ SHALL be registered as a built-in game with canonical description "Fill the grid with filled and empty using some relational clues. No 3 in a row.", aliases `takuzu plus, binario+, binario plus`, variants `Takuzu+ (default 1600 Elo)`, and daily legacy modes `medium, tricky`.
...
The Takuzu+ runtime MUST enforce Takuzu rules plus relation clues, where equals clues require adjacent cells to match and x clues require adjacent cells to differ.
